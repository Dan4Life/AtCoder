...
// state transition is dp[i] = min(dp[i], dp[x]+1) 
// where its possible to form a palindrome from s[x+1...i]
// To speed it up, we can reformulate the problem as:
// each string is a number, such that in the number
// if the ith bit is on, then there is an odd number of occurrences of character i(a=0...z=25)
